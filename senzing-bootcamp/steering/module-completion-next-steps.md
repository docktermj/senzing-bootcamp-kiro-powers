...
### Same Question, Rendered Once

The "**Proceed:** Ready to move on to Module [N] ([name])?" option above and the final-message 👉 "Ready to move on to Module [N]?" transition question are the **same** question, rendered exactly **once** — not two separate prompts. When the forward transition question is itself the final message, do not also render it as a separate inline "Proceed" 👉 line; when a per-module recap/confirmation precedes it, that same question is re-surfaced so the transition question appears a single time as the final message. The "Proceed" wording in the options list only names the choice — it is satisfied by, not added on top of, the one final-message 👉 question.

**De-duplication:** the module-completion turn contains exactly one rendered forward transition 👉 question, matching the single `config/.question_pending` marker written for it. If a recap, next-step summary, or closing line already carries the forward transition question, do not emit a second 👉 copy — the one final-message 👉 question is its only rendering in the turn.
