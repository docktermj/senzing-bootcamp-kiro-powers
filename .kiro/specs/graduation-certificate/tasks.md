...
- [ ] 1. Create script skeleton with CLI interface and data models
  - [ ] 1.1 Create `senzing-bootcamp/scripts/generate_graduation_certificate.py` with argparse CLI, `main(argv=None)` entry point, and all dataclass models (`ProgressData`, `PreferencesData`, `JournalEntry`, `ERMetrics`, `JournalData`, `ModuleRecord`, `CertificateData`)
  ...
  - [ ]* 1.2 Write unit tests for argument parsing
    - Test default values for all arguments
    - Test overriding each argument
    - _Requirements: 2.3, 2.4, 2.5, 2.6_

- [ ] 2. Implement data loaders
  - [ ] 2.1 Implement `load_progress(path)` to parse `bootcamp_progress.json`
  ...
  - [ ] 2.2 Implement `parse_simple_yaml(content)` and `load_preferences(path)` for preferences file
  ...
  - [ ] 2.3 Implement `load_journal(path)` to parse journal markdown
  ...
  - [ ]* 2.4 Write unit tests for data loaders
  ...
  - [ ]* 2.5 Write property test: no unhandled exceptions for any input (Property 8)
...
- [ ] 3. Implement content assembly and logic
  - [ ] 3.1 Implement `assemble_certificate(progress, preferences, journal, project_name)` to build `CertificateData`
  ...
  - [ ]* 3.4 Write property test: skills and next-steps derived from completed track and modules (Property 7)
...
- [ ] 5. Implement renderers
  - [ ] 5.1 Implement `render_markdown(data)` to produce the Markdown certificate
  ...
  - [ ] 5.2 Implement `render_html(data)` to produce the HTML5 certificate
  ...
  - [ ]* 5.3 Write property test: HTML output is valid HTML5 structure (Property 2)
  ...
  - [ ]* 5.4 Write property test: certificate contains identity metadata (Property 3)
  ...
  - [ ]* 5.5 Write property test: all completed modules appear in certificate (Property 4)
  ...
  - [ ]* 5.6 Write property test: journal outcomes appear for matching modules (Property 5)
  ...
  - [ ]* 5.7 Write property test: ER metrics from journal appear in certificate (Property 6)
...
- [ ] 6. Implement file output and wire pipeline together
  - [ ] 6.1 Implement output writing in `main()` — create output directory, write both files
  ...
  - [ ] 6.2 Wire the full pipeline in `main()`: parse args → load → assemble → render → write
    - Top-level exception handler catches unexpected errors, prints to stderr, exits 1
    - Return 0 on success
    - _Requirements: 2.7, 9.4_

  - [ ]* 6.3 Write property test: successful generation produces both output files (Property 1)
  ...
  - [ ]* 6.4 Write property test: idempotent output (Property 9)
...
- [ ] 8. Update steering file for integration
  - [ ] 8.1 Update `senzing-bootcamp/steering/module-completion.md` to invoke the Certificate_Generator
