...
**Render exactly once.** The forward transition 👉 question is rendered exactly once in the turn — it appears a single time, as the single final pending question. The "re-surface after recap/confirmation" path *replaces* (does not duplicate) any earlier inline rendering of the same question: when the forward "Ready for Module X" question is re-surfaced as the final message, do not also leave a separate inline copy of it earlier in the turn (for example, as a distinct "Proceed" 👉 line in the next-step options). Exactly one rendered forward transition 👉 question corresponds to the single `config/.question_pending` marker written for it.
