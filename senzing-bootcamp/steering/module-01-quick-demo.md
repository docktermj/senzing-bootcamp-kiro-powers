...
1. **[SETUP] Create project structure (if needed):** Follow the directory creation commands from the agent-instructions steering file. If the structure doesn't exist, create it before proceeding. After creation, tell the user: "I've created the project directory structure for you. All demo files will be saved in `src/quickstart_demo/`."

2. **[SETUP] Verify Senzing SDK is available:** The SDK should already be installed from Module 0. Verify using a language-appropriate check (see Module 0 Step 1 for examples).

   **If SDK found** → Proceed.

   **If SDK not found** → Direct user to complete Module 0 (SDK Setup) first. Do not proceed without the SDK.

3. **[SETUP] Choose sample dataset:** Call `get_sample_data` to retrieve one of the CORD datasets:
...
4. **[SETUP] Generate demo script:** Call `generate_scaffold` with workflow `full_pipeline` and the bootcamper's chosen language to create a complete demo script that:
...
5. **[SETUP] Save sample data:** Save the sample data retrieved from `get_sample_data` to:

   - `src/quickstart_demo/sample_data_[dataset_name].jsonl`

   Example: `src/quickstart_demo/sample_data_las_vegas.jsonl`

   Tell the user: "Setup is done. Now let's run the demo!"

---

### Phase 2: Demo

1. **[DEMO] Show sample records BEFORE resolution:** Display 3-5 sample records from the dataset. Point out:
...
2. **[DEMO] Run the demo:** Execute the script and show:
...
3. **[DEMO] Display results with match explanations:** After the demo runs, show:
...
4. **[DEMO] Explain the results:** Walk through one resolved entity:
