...
Usage:
    python validate_schema.py --database postgresql --connection "postgresql://user:pass@host:5432/db"
    python validate_schema.py --database sqlite --connection "database/G2C.db"
"""
        ...
        try:
            if self.db_type == 'postgresql':
                if not POSTGRES_AVAILABLE:
                    self.errors.append("psycopg2 not installed. Install with: pip install psycopg2-binary")
                    return False
    ...
    def validate_version_data(self) -> bool:
        ...
        try:
            for var_code, expected_value in self.REQUIRED_VERSIONS.items():
                ...
                if row is None:
                    self.errors.append(f"❌ Missing version entry: SYSTEM.{var_code}")
                    print(f"❌ Missing: {var_code}")
                    all_ok = False
                elif row[0] != expected_value:
                    self.warnings.append(
                        f"⚠️  Version mismatch: {var_code} = '{row[0]}' (expected '{expected_value}')"
                    )
