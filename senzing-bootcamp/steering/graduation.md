...
3. **Emit the announcement exactly once.** Once artifacts are confirmed, emit a single closing announcement to the bootcamper that:
   ...
   - names the rendered-recap PDF path `docs/bootcamp_recap.pdf` and **emphasizes its value** as the bootcamper's keepsake — the shareable record of their whole journey. Make clear what it captures: an opening summary page, then a page per completed module carrying that module's **Information Shared**, **Questions & Responses**, **Actions Taken**, and **Journal**. Frame it as something to keep, revisit, and share with their team — the tangible reward for finishing the bootcamp. (It is always a valid PDF via the tiered strategy — the professionally designed one when `fpdf2` is available, a plainer stdlib-rendered one otherwise; any `docs/bootcamp_recap.html` is only a supplementary extra, never the rendered recap that satisfies the guarantee.)
   ...
   - states that, for every completed module, the recap contains the labeled sections **Information Shared**, **Questions & Responses**, **Actions Taken**, and **Journal**.
...
> 🏆 **Here's your bootcamp trophy.** Your complete recap is captured at `docs/bootcamp_recap.pdf` — a shareable PDF that opens with a summary page and then walks through every module you completed, capturing the **Information Shared**, **Questions & Responses**, **Actions Taken**, and **Journal** for each. Keep it, revisit it, and share it with your team — it's the record of everything you built and learned. The source lives at `docs/bootcamp_recap.md` (your single per-module recap, journal included). Navigate your project from its indexes: `docs/README.md`, `src/README.md`, `data/README.md`, and the top-level `README.md`.
