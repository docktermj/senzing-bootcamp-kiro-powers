...
When resuming a session mid-module via `session-resume.md`, the agent reads `current_step` from `bootcamp_progress.json` (Step 1). If the current module has a `phases` entry in `steering-index.yaml`, use `current_step` to determine the phase and load only that sub-file instead of the full module. If `current_step` is absent or doesn't fall within any phase's `step_range`, load the root file only.
