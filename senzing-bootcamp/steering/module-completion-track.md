...
1. Render the recap PDF from the reconciled recap:
   ...
   This reads the reconciled `docs/bootcamp_recap.md` and overwrites `docs/bootcamp_recap.pdf`. On success, inform the bootcamper and **emphasize its value** — this PDF is their keepsake and the tangible reward for their progress: an opening summary page followed by a page per completed module capturing that module's **Information Shared**, **Questions & Responses**, **Actions Taken**, and **Journal**. For example: "🏆 Your recap PDF is ready at `docs/bootcamp_recap.pdf` — a shareable record of everything you've built and learned so far, with a summary page and a page for each module. Keep it, revisit it, and share it with your team."
...
> **Note (enforced guarantee):** The always-run renders above are the *best-effort* pass; they stay non-blocking so a render failure never holds up the celebration. The recap PDF / transcript / recap Markdown are additionally **guaranteed** at every stopping point by the `enforce-critical-artifacts` `Stop` hook. That hook is a **deterministic command hook**: on every agent stop it runs `python3 senzing-bootcamp/scripts/ensure_graduation_artifacts.py --stop-hook`, which itself decides whether a stopping point has been reached (module 7 or 11 completed) and whether a question is pending (`config/.question_pending`) before regenerating any absent, empty, or stale artifact. Because the runtime runs the script directly — rather than relying on the agent to act on a prompt — the artifacts are produced whether or not the agent cooperates. The orchestrator preserves the reconcile-then-render ordering used here (reconcile the source, then render) and guarantees a valid `docs/bootcamp_recap.pdf` via the tiered strategy (the best-effort `fpdf2` auto-install may run; otherwise the stdlib-only PDF writer is used), with a no-data floor so a valid PDF is produced even when no module content was captured. Any `docs/bootcamp_recap.html` is only a supplementary extra, never the artifact that satisfies the rendered-recap guarantee.
