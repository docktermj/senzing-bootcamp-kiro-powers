...
Every artifact-creation step (consolidated recap append, completion certificate) MUST handle errors gracefully so that a single failure never blocks the bootcamper's flow.

### Per-Step Error Handling

For each of the following steps — **consolidated_append** and **completion_certificate** — apply these rules:
...
2. **Log a warning** visible to the bootcamper using this exact format:
   ...
   Examples:
   - `⚠️ consolidated_append skipped: Permission denied writing to docs/bootcamp_recap.md. This will be retried on next module completion.`
   - `⚠️ consolidated_append skipped: Disk full. This will be retried on next module completion.`
...
5. **Do NOT mark a successful step as skipped** — when the consolidated append writes its section successfully, treat the append as complete and log no skip warning for it.
...
- The completion certificate does NOT depend on the consolidated append's output — it can proceed independently.
- Even if the consolidated append fails or is intentionally skipped, the **completion_certificate** and **next_step_options** steps MUST still execute and present the bootcamper with their choices.
