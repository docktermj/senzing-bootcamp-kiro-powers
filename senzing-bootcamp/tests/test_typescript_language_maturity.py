...
class TestOnboardingDisclaimerPresence:
    ...
    while `generate_scaffold` produces working code for all. After the preface
    reorder (track before language), this step lives in
    onboarding-phase2-track-setup.md as Step 5a (after Track Selection).

    **Validates: Requirements 6**
    """

    def _get_step2_content(self) -> str:
        """Extract the Programming Language Selection (Step 5a) section content."""
        file_path = _STEERING_DIR / "onboarding-phase2-track-setup.md"
        content = file_path.read_text(encoding="utf-8")

        step2_match = re.search(
            r"## 5a\. Programming Language Selection\s*\n(.*?)(?=\n## \d|\Z)",
            content,
            re.DOTALL,
        )
        assert step2_match is not None, (
            "onboarding-phase2-track-setup.md is missing "
            "'## 5a. Programming Language Selection' section"
        )
