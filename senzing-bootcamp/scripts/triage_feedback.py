...
def generate_bugfix_skeleton(entry: FeedbackEntry) -> str:
    ...
    if entry.why_problem:
        lines.append(
            f"The expected behavior is the opposite of the reported problem: {entry.why_problem}"
        )
    else:
...
def generate_requirements_skeleton(entry: FeedbackEntry) -> str:
    ...
    lines.append(
        "<!-- Auto-generated by triage_feedback.py. "
    ...
    if entry.suggested_fix:
        lines.append(
            f"**User Story:** As a user, I want {entry.suggested_fix.lower().rstrip('.')}, "
            f"so that the experience is improved."
        )
    else:
