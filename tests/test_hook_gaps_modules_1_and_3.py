...
VALIDATE_HOOK_PATH = HOOKS_DIR / "validate-business-problem.json"
VERIFY_HOOK_PATH = HOOKS_DIR / "verify-demo-results.json"
...
def validate_hook_data() -> dict:
    """Load and parse the validate-business-problem v1 hook entry."""
...
def verify_hook_data() -> dict:
    """Load and parse the verify-demo-results v1 hook entry."""
...
class TestHookFileStructure:
    ...
    def test_validate_hook_is_valid_json(self):
        """validate-business-problem.json parses as a valid v1 wrapper."""
        with open(VALIDATE_HOOK_PATH, encoding="utf-8") as f:
            data = json.load(f)
    ...
    def test_verify_hook_is_valid_json(self):
        """verify-demo-results.json parses as a valid v1 wrapper."""
        with open(VERIFY_HOOK_PATH, encoding="utf-8") as f:
            data = json.load(f)
        assert isinstance(data, dict)
        assert data.get("version") == "v1"
    ...
    def test_validate_hook_version(self):
        """validate-business-problem wrapper declares the v1 schema version."""
        with open(VALIDATE_HOOK_PATH, encoding="utf-8") as f:
            assert json.load(f)["version"] == "v1"

    def test_validate_hook_has_description(self, validate_hook_data: dict):
        """validate-business-problem has a non-empty action prompt (no description in v1)."""
        assert len(validate_hook_data["action"]["prompt"]) > 0

    def test_validate_hook_when_type(self, validate_hook_data: dict):
        """validate-business-problem uses the PostTaskExec trigger."""
        assert validate_hook_data["trigger"] == "PostTaskExec"

    def test_validate_hook_then_type(self, validate_hook_data: dict):
        """validate-business-problem uses the agent action."""
        assert validate_hook_data["action"]["type"] == "agent"

    def test_validate_hook_has_prompt(self, validate_hook_data: dict):
        """validate-business-problem has a non-empty prompt."""
        assert len(validate_hook_data["action"]["prompt"]) > 20
    ...
    def test_verify_hook_version(self):
        """verify-demo-results wrapper declares the v1 schema version."""
        with open(VERIFY_HOOK_PATH, encoding="utf-8") as f:
            assert json.load(f)["version"] == "v1"

    def test_verify_hook_has_description(self, verify_hook_data: dict):
        """verify-demo-results has a non-empty action prompt (no description in v1)."""
        assert len(verify_hook_data["action"]["prompt"]) > 0

    def test_verify_hook_when_type(self, verify_hook_data: dict):
        """verify-demo-results uses the PostTaskExec trigger."""
        assert verify_hook_data["trigger"] == "PostTaskExec"

    def test_verify_hook_then_type(self, verify_hook_data: dict):
        """verify-demo-results uses the agent action."""
        assert verify_hook_data["action"]["type"] == "agent"

    def test_verify_hook_has_prompt(self, verify_hook_data: dict):
        """verify-demo-results has a non-empty prompt."""
        assert len(verify_hook_data["action"]["prompt"]) > 20
...
class TestValidateBusinessProblemPrompt:
    ...
    def test_module_guard_reads_progress(self, validate_hook_data: dict):
        ...
        prompt = validate_hook_data["action"]["prompt"]
    ...
    def test_silent_when_not_module_1(self, validate_hook_data: dict):
        ...
        prompt = validate_hook_data["action"]["prompt"].lower()
...
class TestVerifyDemoResultsPrompt:
    ...
    def test_module_guard_reads_progress(self, verify_hook_data: dict):
        """Prompt instructs reading config/bootcamp_progress.json."""
        prompt = verify_hook_data["action"]["prompt"]
        assert "config/bootcamp_progress.json" in prompt
    ...
    def test_silent_when_not_module_3(self, verify_hook_data: dict):
        ...
        prompt = verify_hook_data["action"]["prompt"].lower()
        assert "produce no output" in prompt
