...
- [x] Open `senzing-bootcamp/steering/agent-instructions.md`
- [x] In the `## MCP Rules` section, after the line about `get_sdk_reference` for signatures, add: `- SDK method calls that accept flags: look up available flags via get_sdk_reference(topic='flags'), select flags matching the bootcamper's intent, and explain the choice in one sentence. Reuse flag knowledge within a module session. Default flags are acceptable for simple lookups but note that detailed flags exist.`
- [x] Verify the file still parses correctly (valid markdown, no broken formatting)

## Task 2: Add Flag Selection Protocol to mcp-tool-decision-tree.md

- [x] Open `senzing-bootcamp/steering/mcp-tool-decision-tree.md`
- [x] After the `## Call Pattern Examples` section's `get_sdk_reference` example, add a new section `## Flag Selection Protocol` with the following content:
...
- [x] Add an anti-pattern row to the existing anti-patterns table: "Instead of: Passing None/default flags without checking | Use: `get_sdk_reference(topic='flags')` | Consequence: Missing detail needed for visualizations, no teaching moment about flag system"

## Task 3: Add flag-aware instruction to module-07-query-validation.md

- [x] Open `senzing-bootcamp/steering/module-07-query-validation.md`
- [x] In Step 2 (Create query programs), after the `generate_scaffold` instruction, add an agent instruction block:
  ```
  ...
  ```
- [x] In Step 3a (Present query results), add a note that when presenting results from `how_entity` or `why_*` methods, the agent should have used feature score and match key detail flags to provide informative output

## Task 4: Update steering-index.yaml token counts

- [x] Run `python3 senzing-bootcamp/scripts/measure_steering.py` to recalculate token counts for the three modified files
- [x] Update `steering-index.yaml` `file_metadata` entries for `agent-instructions.md`, `mcp-tool-decision-tree.md`, and `module-07-query-validation.md` with new token counts
- [x] Verify no file crossed a `size_category` boundary (all should remain in their current category)
...
- [x] Run `python3 senzing-bootcamp/scripts/validate_commonmark.py` to confirm all modified markdown files are valid CommonMark
- [x] Run `python3 senzing-bootcamp/scripts/measure_steering.py --check` to confirm token budgets are within limits
- [x] Run `pytest senzing-bootcamp/tests/` to confirm no test failures
