...
class TestHookCount:
    ...
    def test_hook_file_count(self):
        """Exactly 17 .kiro.hook files exist (Req 5.1, 5.3)."""
    ...
    def test_registry_entry_count(self):
        """Exactly 17 registry entries exist (Req 5.2, 5.3)."""
...
class TestRealHookFiles:
    ...
    def test_all_18_hook_files_parse_as_valid_json(self):
        """All 17 real hook files parse as valid JSON (Req 1.1)."""
    ...
    def test_hook_file_count_is_18(self):
        """Hook file count is exactly 17 (Req 5.1)."""
        assert len(get_hook_files()) == 17

    def test_registry_entry_count_is_18(self):
        """Registry entry count is exactly 17 (Req 5.2)."""
        assert len(parse_registry()) == 17
    ...
    def test_registry_names_match_file_names(self):
        """Registry names match file names for all 17 hooks (Req 4.4)."""
    ...
    def test_registry_descriptions_match_file_descriptions(self):
        """Registry descriptions match file descriptions for all 17 hooks (Req 4.5)."""
        for hook_id in _common_ids:
            file_desc = _hook_data_by_id[hook_id]["description"]
            registry_desc = _registry_by_id[hook_id].description
            assert file_desc == registry_desc, (
