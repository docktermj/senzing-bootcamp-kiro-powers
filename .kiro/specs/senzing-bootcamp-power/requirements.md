...
1. THE Power SHALL provide 29 hooks organized into critical (created during onboarding), module-specific (created when the module starts), and any-module categories.
...
4. ANY-MODULE hooks (6) SHALL include: backup-project-on-request, error-recovery-context, git-commit-reminder, module-completion-celebration, module-recap-append, session-log-events.
5. THE hook registry (`hook-categories.yaml`, `hook-registry.md`, `hook-registry-critical.md`, the per-module `hook-registry-module-NN.md` files, and `hook-registry-module-any.md`) SHALL stay in sync with `.kiro.hook` files — verified by `sync_hook_registry.py --verify`.
