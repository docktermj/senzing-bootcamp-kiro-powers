...
# ---------------------------------------------------------------------------
# 8.2 Parse all 18 real hook files without errors (Req 1.1)
# ---------------------------------------------------------------------------
...
class TestParseAllRealHooks:
    ...
    def test_all_18_hooks_parse_without_errors(self):
        entries, errors = parse_all_hooks(_HOOKS_DIR)
        assert len(errors) == 0, f"Parse errors: {errors}"
        assert len(entries) == 19, f"Expected 19 hooks, got {len(entries)}"
...
class TestCategoryMappingLoads:
    ...
    def test_load_real_categories(self):
        ...
        # Should have all 19 hooks mapped
        assert len(mapping) == 19, f"Expected 19 mappings, got {len(mapping)}"

        # Check some known critical hooks
        assert mapping["ask-bootcamper"].category == "critical"
        assert mapping["capture-feedback"].category == "critical"
