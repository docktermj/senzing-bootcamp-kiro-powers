...
Immediately **after** the `hooks_installed` check above, inspect the bootcamper's `.kiro/hooks` directory for the two **capture-critical** hooks — `session-log-events` and `ask-bootcamper`. They feed the completion summary and recap (`ask-bootcamper` Phase 0 appends the module recap); if either is missing, that output is silently incomplete.
...
- Run the file-copy installer: `python3 senzing-bootcamp/scripts/install_hooks.py --essential` (its `--essential` set includes both).
